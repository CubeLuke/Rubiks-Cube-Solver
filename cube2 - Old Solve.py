...
import time, copy, webbrowser, os
from tkinter import *
...
#Puts the appropriate corner in the front-right slot.
#Part of old F2L
def put_corner():
    for i in range(4):
        if i == 1:
        ...
        if i == 3:
            m("Li Ui L")
        for j in range(4): 
            if a[4][0][2] == a[4][1][1] and a[1][2][2] == a[1][1][1] and a[2][2][2] == a[2][1][1]:
                #print("Corner is already done")
                return
            if a[4][0][2] == a[2][1][1] and a[1][2][2] == a[4][1][1] and a[2][2][2] == a[1][1][1]:
                m("Fi Ui F U Fi Ui F")
                return
            if a[4][0][2] == a[1][1][1] and a[1][2][2] == a[2][1][1] and a[2][2][2] == a[4][1][1]:
                m("R U Ri Ui R U Ri")
                return
            if a[1][0][2] == a[4][1][1] and a[0][2][2] == a[1][1][1] and a[2][2][0] == a[2][1][1]:
                if a[0][1][2] == a[1][1][1] and a[2][1][0] == a[2][1][1]:
                    m("U R Ui Ri")
                    return
                else:
                    m("Fi Ui F")
                    return
            if a[1][0][2] == a[1][1][1] and a[0][2][2] == a[2][1][1] and a[2][2][0] == a[4][1][1]:
                if a[1][0][1] == a[1][1][1] and a[0][2][1] == a[2][1][1]:
                    m("Ui Fi U F")
                    return
                else:
                    m("R U Ri")
                    return
            if a[1][0][2] == a[2][1][1] and a[0][2][2] == a[4][1][1] and a[2][2][0] == a[1][1][1]:
                m("R U2 Ri Ui R U Ri")
                return
            m("U")

#Solves all four corners by placing each one
#Part of the old F2L
def corners():
    for i in range(4):
        put_corner()
        m("Y")
    assert a[4][0][0] == a[4][0][2] == a[4][2][0] == a[4][2][2] == a[4][1][1]

#Places the appropriate edge piece in the front-right slot
#Part of the old F2L
def put_edge():
    for i in range(4):
        if i == 1:
            m("Y R U Ri Ui Fi Ui F Yi Ui")
        if i == 2:
            m("Y2 R U Ri Ui Fi Ui F Y2")
        if i == 3:
            m("Yi R U Ri Ui Fi Ui F Y U")
        for j in range(4):
            if a[1][1][2] == a[1][1][1] and a[2][1][1] == a[2][2][1]:
                return
            m("R U Ri Ui Fi Ui F Ui R U Ri Ui Fi Ui F Ui")
            if a[1][1][2] == a[1][1][1] and a[2][1][1] == a[2][2][1]:
                return
            m("U R U Ri Ui Fi Ui F")
    assert a[1][1][2] == a[1][1][1] and a[2][1][1] == a[2][2][1]

#Places all four corners in the correct spot.
#Part of the old F2L
def edges():
    #this first step ensures that all remaining pieces are on the U layer
    for i in range(4):
        #check front left, see if it belongs anywhere
        #swap with opposite and flip
        if a[3][2][1] == a[5][1][1] and a[1][1][0] == a[2][1][1]:
            m("Ri Li U L Ri Fi L F L2 R2 Bi R B L Ri")
        #swap with opposite but don't flip
        if a[3][2][1] == a[2][1][1] and a[1][1][0] == a[5][1][1]:
            m("F2 R2 U2 R2 U2 R2 U2 F2 U2 L Ri F2 R Li")
        #swap with left-back, don't flip
        if a[3][2][1] == a[3][1][1] and a[1][1][0] == a[5][1][1]:
            m("L2 U2 L2 U2 L2")
        #swap with left-back, flip
        if a[3][2][1] == a[5][1][1] and a[1][1][0] == a[3][1][1]:
            m("Yi R Li U L Ri Fi L F L2 R2 Bi Ri B L Ri Y")
        #swap with front-right, don't flip
        if a[3][2][1] == a[2][1][1] and a[1][1][0] == a[1][1][1]:
            m("F2 U2 F2 U2 F2")
        #swap with front-right, flip
        if a[3][2][1] == a[1][1][1] and a[1][1][0] == a[2][1][1]:
            m("R Li U L Ri Fi L F L2 R2 Bi Ri B L Ri")
            
        #check front right, see if it belongs anywhere
        #swap with opposite and flip
        if a[1][1][2] == a[3][1][1] and a[2][2][1] == a[5][1][1]:
            m("Yi Ri Li U L Ri Fi L F L2 R2 Bi R B L Ri Y")
        #swap with opposite but don't flip
        if a[1][1][2] == a[5][1][1] and a[2][2][1] == a[3][1][1]:
            m("Y F2 R2 U2 R2 U2 R2 U2 F2 U2 L Ri F2 R Li Yi")
        #swap with right-back, don't flip
        if a[1][1][2] == a[5][1][1] and a[2][2][1] == a[2][1][1]:
            m("R2 U2 R2 U2 R2")
        #swap with right-back, flip
        if a[1][1][2] == a[2][1][1] and a[2][2][1] == a[5][1][1]:
            m("Y R Li U L Ri Fi L F L2 R2 Bi Ri B L Ri Yi")
        #swap with front-left, don't flip
        if a[1][1][2] == a[1][1][1] and a[2][2][1] == a[3][1][1]:
            m("F2 U2 F2 U2 F2")
        #swap with front-left, flip
        if a[1][1][2] == a[3][1][1] and a[2][2][1] == a[1][1][1]:
            m("R Li U L Ri Fi L F L2 R2 Bi Ri B L Ri")
        m("Y")

	#now all remaining edges are in the U layer, so just insert them (or flip if needed)
    count = 0
    while count < 4:
        #if a[1][1][2] == a[1][1][1] and a[2][2][1] == a[2][1][1]: #if it's already solved
        #    count += 1
        #    continue
        if a[1][1][2] == a[2][1][1] and a[2][2][1] == a[1][1][1]: #or needs to be flipped
            m("R U Ri Ui Fi Ui F Ui R U Ri Ui Fi Ui F")
            count = 0
            continue
        
        #otherwise, find the proper piece in the U layer and insert
        for j in range(4):
            if a[1][0][1] == a[1][1][1] and a[0][2][1] == a[2][1][1]:
                m("U R U Ri Ui Fi Ui F")
                count = 0
                break
            if a[1][0][1] == a[2][1][1] and a[0][2][1] == a[1][1][1]:
                m("U2 Fi Ui F U R U Ri")
                count = 0
                break
            m("U")
        m("Y")
        count += 1
    flSame = a[1][1][0] == a[1][1][1] and a[3][2][1] == a[3][1][1]
    frSame = a[1][1][2] == a[1][1][1] and a[2][2][1] == a[2][1][1]
    blSame = a[3][0][1] == a[3][1][1] and a[5][1][0] == a[5][1][1]
    brSame = a[2][0][1] == a[2][1][1] and a[5][1][2] == a[5][1][1]
    assert all([flSame, frSame, blSame, brSame])
...
#Will return the loction of the edge that belongs in the FR spot. Either returns BR, BL, FL, or FR.
def f2lEdgeCheck():
...
#Old solve uses our beginners method f2l
def oldSolve():
    cross()
    corners()
    edges()
...
def GUIy():
    ...
    scram = StringVar(value="Enter custom Scramble")
    ...
    ScrambleButton = Button(frame, text="Scramble", command = lambda: GUIScramble()).grid(row = 5, column = 0)
    CopyScrambleButton = Button(frame, text="Copy Scramble", command = lambda: GUItoClipboard(get_scramble())).grid(row = 5, column = 1)
    ...
    CustomScramButton = Button(frame,text="Custom Scramble", command = lambda: GUIcustomScramble(scram))
    ...
    SolveButton = Button(frame, text="Solve",bg="#D53",command = lambda: GUISolve()).grid(row = 7, column = 0) #sticky="e" if using timer button as well
    CopyScrambleButton = Button(frame, text="Copy Solution", command = lambda: GUItoClipboard(get_moves())).grid(row = 7, column = 1)
